...
echo "== tramo · cuenta $CUENTA · sesion $SESION · $UNI · +$TRAMO de $PASOS pasos · p_nose $P_NOSE · abst $ABST · donde $DONDE · mezcla $MEZCLA · piso $MEZCLA_PISO · p_vieja $P_VIEJA · blanco $BLANCO · kq $KERNEL_Q · ses-extra $SES_EXTRA · sello $SELLO · pert $PERT · micro $MICRO_BATCH · b_eval $BATCH_EVAL · formas $FORMAS_Q · perdida $PERDIDA_CABEZA · sembrar $SEMBRAR · rec L=$REC_L M=$REC_M F=$REC_F CE=$REC_CE RANK=$REC_RANK · topk $TOPK desde $TOPK_DESDE · relleno $RELLENO ($RELLENO_DIST, $RELLENO_TURNOS, cada $RELLENO_CADA) · fotos $FOTOS · ses-extra-sin-grad $SES_EXTRA_SIN_GRAD · compuesta $P_COMPUESTA · bloques $BLOQUES_LECTURA · rel2-sesion $REL2_SESION"
