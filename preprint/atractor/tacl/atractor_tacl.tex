% ENVIO A TACL · version ANONIMA. El estilo en modo `submission` (el default) numera
% las lineas y suprime autor y afiliacion, que es lo que pide la revision doble ciego.
\documentclass[11pt]{article}
\usepackage[hyperref]{tacl2021v1}   % el default YA es la version de submission; el .sty documenta una opcion `submission` que NO declara y pasarla da error
\usepackage{times}
\usepackage{latexsym}
...
\title{Perfect Abstention Is Zero Knowledge: Two Stable Fixed Points of a
Self-Referential Abstention Objective in a Small Language Model with Versioned Memory}

% El estilo de TACL suprime esto en modo `submission`. Se deja para la version final.
\author{Anonymous}
...
\citep{cohen2024idk} and adding a separate binary head are the two dominant interfaces; a
pre-registered four-way comparison under matched budget \citep{speranza2026abstention} found the head
...
and machine-generated reports are archived in a public project repository whose commit history
carries the timestamps that establish the order of pre-registration and result. The repository URL
identifies the author and is therefore withheld for double-blind review; an anonymized mirror is
provided with the submission and the full URL will be given on acceptance.

\section*{Declarations}

\textbf{Funding.} None. \textbf{Competing interests.} None. \textbf{Ethics approval.} Not applicable;
no human or animal subjects. \textbf{Data availability.} See above.
