...
time.sleep(15)
sh("ollama pull gemma:2b 2>&1 | tail -1")
...
textos = [x["v1"] for x in items] + [x["v2"] for x in items] + [x["consulta"] for x in items]
vecs, t0, fallos = [], time.time(), 0
with ThreadPoolExecutor(max_workers=8) as ex:
    for i, v in enumerate(ex.map(embed, textos)):
        if v is None:
            fallos += 1; vecs.append([0.0] * 2048)
        else:
            vecs.append(v)
        if (i + 1) % 1000 == 0:
